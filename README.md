...
La comunicacion del cliente al servidor es un protocolo binario donde se envia cada campo precedido de su longitud.
La comunicacion del servidor al cliente es un protocolo de texto donde se envia la informacion con un delimitador que indica el final del mensaje y un separador que sirve para separar los campos.
...
Por lo cual un mensaje de apuesta seria por ejemplo: 6CAMILA5AYALA842915189102001-01-1547475 pero pasado a binario.
...
Se intenta leer del socket esa cantidad de bytes para llenar un campo. Una vez que se llenan todos los campos se finaliza la lectura.
...
Si el cliente recibe algo distinto a ese mensaje sabe que hubo un error y va a loggear lo que el servidor le mando, que sera del tipo "ERROR: {descripcion del error que sucedio}"
...
### ej6:
Se envia al servidor una cantidad de apuestas que va a haber en el batch antes de las apuestas.
...
### ej7:
Ahora el cliente debe identificarse con el servidor agregando al comienzo de su mensaje su agency_id. Su longitud esta determinada por la constante AGENCY_ID_BYTES. Del lado del cliente se la encodea usando la funcion GetU8Bytes pero de desear cambiarlo por un numero mayor no deberia necesitarse mas que agregar una nueva funcion del tipo GetU32Bytes y encodeandolo con esta. 
...
Los mensajes de NOTIFY y REQ_RESULTS no tienen un payload. El mensaje de PLACE_BETS tiene el mismo payload que antes: BATCH_SIZEapuesta1..apuestaN. 
...
Para procesar mensajes en paralelo use threads de la libreria threading de Python. 

Debido a que las tareas a paralelizar son mayormente compuestas de operaciones bloqueantes del tipo I/O (lectura de socket del cliente, escritura y lectura del archivo de apuestas) y el GLI (Global Lock Interpreter) no bloquea el progreso de otros hilos durante estas operaciones, considere que el uso de threads tendria una performance similar a una de multiprocessing. 

Se utilizo el modelo de threadpool, en el cual: 
...
Por otro lado, para coordinar el acceso a los recursos compartidos y sincronizar a los hilos se utilizo:
